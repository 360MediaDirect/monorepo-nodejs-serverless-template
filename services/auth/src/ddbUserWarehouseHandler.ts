...
import { ddbSchemaUser } from './models/UserModel'
...
export const fire: Handler = async (event, _context, _callback) => {
  ...
  await asyncForEach(Records, async (record: DynamoDBRecord) => {
    ...
    if (!record.dynamodb?.NewImage?.id?.S) {
      log.warn(
        'Unexpected DDB streams message in auth.ddbUserWarehouseHandler',
        {
          record,
        },
      )
    ...
    const msg = {
      ...
      Message: JSON.stringify({
        tableName: process.env.USER_WAREHOUSE_TABLE_NAME,
        entityRecord: unmarshallItem(ddbSchemaUser, record.dynamodb?.NewImage),
        cKeys: ['id'],
      }),
      MessageDeduplicationId: `${record.dynamodb?.NewImage?.id?.S}_${record.dynamodb?.NewImage?.updatedAt.N}`,
      MessageGroupId: `${process.env.USER_WAREHOUSE_TABLE_NAME}_${record.dynamodb?.NewImage?.id?.S}`,
    }
