...
import { ddbSchemaUserId } from './models/UserIdentifierModel'
...
export const fire: Handler = async (event, _context, _callback) => {
  ...
  await asyncForEach(Records, async (record: DynamoDBRecord) => {
    ...
    if (!record.dynamodb?.NewImage?.id?.S) {
      log.warn(
        'Unexpected DDB streams message in auth.ddbUserIdWarehouseHandler',
        {
          record,
        },
      )
    ...
    const msg = {
      ...
      Message: JSON.stringify({
        tableName: process.env.USER_ID_WAREHOUSE_TABLE_NAME,
        entityRecord: unmarshallItem(
          ddbSchemaUserId,
      ...
      MessageGroupId: `${process.env.USER_ID_WAREHOUSE_TABLE_NAME}_${record.dynamodb?.NewImage?.id?.S}`,
    }
